...
def load_posted_urls():
    try:
        ...
        if len(lines) > 500:
            ...
            print("[STATE] Trimmed posted_urls.txt to 500 entries")

        return set(lines)
    except FileNotFoundError:
        return set()
    except Exception as error:
        print(f"[STATE] Could not read {POSTED_FILE}: {error}")
        return set()
...
def save_posted_url(url):
    ...
    try:
        ...
        print(f"[STATE] Saved posted URL ({len(existing)}/500)")
    except Exception as error:
        print(f"[STATE] Could not save URL: {error}")
...
def build_shortlist_prompt(news):
    ...
    for index, item in enumerate(news, start=1):
        blocks.append(
            ...
            f"Title: {item['title']}\n"
            f"URL: {item['url']}\n"
            f"Summary: {truncate_text(item.get('summary', ''), 900)}\n"
        )
    joined = "\n".join(blocks)
...
def build_selection_prompt(candidates):
    ...
    for index, item in enumerate(candidates, start=1):
        article_text = item.get("text", "") or item.get("summary", "")
        article_text = truncate_text(article_text, MAX_SELECTION_ARTICLE_LENGTH)
        blocks.append(
            f"NEWS {index}\n"
            f"Source: {item['source']}\n"
            f"Title: {item['title']}\n"
    ...
    joined = "\n".join(blocks)
    return render_prompt(SELECTION_PROMPT, CANDIDATES=joined)
...
def title_is_bad(title):
    ...
    if lowered in generic or len(value) < 20:
        return True
    words = re.findall(r"[A-Za-zА-Яа-яЁё0-9][A-Za-zА-Яа-яЁё0-9-]*", value)
    russian_letters = len(re.findall(r"[А-Яа-яЁё]", value))
    latin_letters = len(re.findall(r"[A-Za-z]", value))
    if russian_letters == 0 and latin_letters > 12:
        return True
    if len(words) < 4:
        return True
    return False
...
def clean_generated_post(post):
    ...
    for line in post.splitlines():
        if re.match(r"^\s*(Источник|Source)\s*:", line, re.IGNORECASE):
            continue
        if re.match(r"^\s*(Оригинал|Original)\s*$", line, re.IGNORECASE):
            continue
...
def generate_telegram_post(article):
    print("[AI] Writing post...")
    response = gemini_request(
        prompt=build_post_prompt(article),
    ...
    if not result:
        print("[AI] Post generation: could not parse response")
        return None

    title = clean_text(result.get("title", ""))
    post = clean_generated_post(result.get("post", ""))
    if not title or not post:
        print("[AI] Post generation: empty title or post")
        return None

    if title_is_bad(title):
        print(f"[AI] Title rejected: {title}")
        repair_prompt = render_prompt(
            POST_REPAIR_PROMPT,
            ARTICLE_TITLE=article.get("title", ""),
            ARTICLE_SOURCE=article.get("source", ""),
            ARTICLE_TEXT=truncate_text(article.get("text", "") or article.get("summary", ""), MAX_POST_ARTICLE_LENGTH),
            BAD_TITLE=title,
            POST=post,
        )
        repair_response = gemini_request(
        ...
        if repair_response:
            repaired = parse_json_response(repair_response)
            if repaired:
                repaired_title = clean_text(repaired.get("title", ""))
                repaired_post = clean_generated_post(repaired.get("post", ""))
                if repaired_title and repaired_post and not title_is_bad(repaired_title):
                    title, post = repaired_title, repaired_post
...
def verify_image_with_gemini(article, image_item):
    image_bytes = image_item.get("data")
    if not image_bytes:
        return False, 0, "No image bytes"

    prompt = render_prompt(
        IMAGE_VERIFY_PROMPT,
        ARTICLE_TITLE=article.get("title", ""),
        ARTICLE_TEXT=truncate_text(article.get("text", "") or article.get("summary", ""), 5000),
        IMAGE_ALT=image_item.get("alt", ""),
        IMAGE_SOURCE=image_item.get("source", ""),
    )
    try:
        image_format = str(image_item.get("format", "JPEG")).lower()
        mime_type = {
            "jpeg": "image/jpeg",
            "jpg": "image/jpeg",
            "png": "image/png",
        ...
        config = types.GenerateContentConfig(
            response_mime_type="application/json",
            response_schema=IMAGE_VERIFY_SCHEMA,
            temperature=0.0,
        ...
        if not result:
            return False, 0, "Could not parse vision response"
        ...
        return relevant and score >= 65, score, reason
    except Exception as error:
        print(f"[IMAGE AI] Verification error: {error}")
        return False, 0, str(error)


def get_article_image(article):
    print("[IMAGE] Searching for best image...")
    candidates = []
    seen = set()

    for candidate in extract_rss_image_candidates(article.get("rss_entry", {})):
        append_image_candidate(candidates, seen, candidate["url"], candidate["source"], article["url"], candidate.get("alt", ""))

    page_html = article.get("page_html")
    if not page_html:
        try:
            page_html = fetch_article_page(article["url"])
            article["page_html"] = page_html
        except Exception as error:
            print(f"[IMAGE] Could not fetch article page: {error}")

    if page_html:
        for candidate in extract_page_image_candidates(page_html, article["url"], article["title"]):
            append_image_candidate(candidates, seen, candidate["url"], candidate["source"], article["url"], candidate.get("alt", ""))

    if not candidates:
        print("[IMAGE] No image candidates")
        return None

    scored = []
    for candidate in candidates[:MAX_IMAGE_CANDIDATES]:
        image_bytes = download_image_bytes(candidate["url"])
        if not image_bytes:
            continue
        image_info = inspect_image_bytes(image_bytes)
        if not image_info:
            continue
        score = score_image_candidate(candidate, image_info, article["title"])
        if score < 0:
            continue
        scored.append({
            "url": candidate["url"],
    ...
    if not scored:
        print("[IMAGE] No usable images")
        return None

    scored.sort(key=lambda item: (item["score"], item["area"]), reverse=True)
    verified = []
    for item in scored[:6]:
        ok, semantic_score, reason = verify_image_with_gemini(article, item)
        print(f"[IMAGE AI] {item['source']} | score={semantic_score} | relevant={ok} | {reason}")
        if ok:
            item["semantic_score"] = semantic_score
            item["final_score"] = item["score"] + semantic_score * 1.5
            verified.append(item)

    if not verified:
        print("[IMAGE] Gemini rejected all top candidates")
        return None

    verified.sort(key=lambda item: (item["final_score"], item["area"]), reverse=True)
    best = verified[0]
    print(
        f"[IMAGE] Selected: {best['source']} | {best['width']}x{best['height']} | "
        f"heuristic={best['score']} | semantic={best['semantic_score']}"
    )
    return best
